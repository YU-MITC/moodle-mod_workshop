                    ...
                    if ($files  = $fs->get_area_files($this->context->id, 'mod_workshop', 'submission_attachment', $submission->id)) {
                      foreach($files as $file){
                          if ($file->get_mimetype() && $file->get_filename() != ".") {
                              ...
                              // For the Windows and Japanese environments, convert filename encoding to Shift JIS.
                              $lang = current_language();
                              if (core_useragent::check_browser_operating_system('Windows') && $lang == 'ja') {
                                  $prefixedfilename = mb_convert_encoding($prefixedfilename, "SJIS");
                              }
